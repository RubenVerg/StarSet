...
instance Game Stun where
  ...
  rules Stun line ex =
    ...
    , line "A set is a group of three cards where, for each characteristic, two cards share a trait and the third has a different one."
    , line "For example, this is a set:"
    , ex
      [ SetCard Two Empty Tilde Green
      , SetCard Three Filled Oval Red
      , SetCard Three Filled Tilde Green
      ]
    ]
